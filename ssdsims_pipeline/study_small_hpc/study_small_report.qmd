...
20,500 fits. This section sets out the mechanism, uses the per-source figures to
show that the degeneracy is a property of the criterion rather than of the data, and
reports one result from the earlier grey literature that cannot be reconciled with
this run. A comparison establishing that the degeneracy is invariant to `ssdtools`
version and to the fit arguments is given in
...
### Per-source weights across the 41 datasets

The two figures below track, for data generated from each source distribution, how
the re-fitted AICc weights change with $N$: @fig-4a the weight assigned to the
source distribution itself, @fig-4b the weight assigned to the `lnorm_lnorm`
mixture. Both are built from the committed `source_weights_summary.csv` (mean
re-fitted AICc weight per source × $N$ × distribution, averaged over the 500
simulations and over the datasets sharing a source), and their per-dataset spread
from `fit_weights_summary.csv` and `fit_weights_quantiles.csv`. Panel headers carry
the number of contributing datasets so that thin panels — the `lnorm_lnorm` source,
in particular, has only one — are visible. Together they turn the single-dataset
reprex above into a result: the same mechanism holds across all 41 datasets.
...
```{r}
#| label: fig-4b
#| fig-cap: "Re-fitted AICc weight of the lnorm_lnorm mixture versus N, faceted by source distribution. Each point is one dataset's mean weight over its 500 simulations; each grey line is that dataset's 10th–90th simulation percentile. At N = 5 every point sits at ≈ 1 with a negligible range in every panel — including all five unimodal sources, which did not generate mixture data. The weight drops to exactly 0 at N = 6 (AICc = +∞) and only becomes meaningful from N = 7 (dashed line), where a non-trivial mixture weight requires genuine bimodality in the source data and a sample large enough to detect it."
#| fig-width: 8
#| fig-height: 5
...
Above the degenerate range the figures read the way model averaging should. In
@fig-4a, once the mixture's AICc becomes valid at $N \ge 7$ the weight on the source
distribution climbs steadily with $N$: at small $N$ a handful of observations cannot
distinguish one candidate distribution from another, so the weight is spread across
them, and it concentrates on the generating distribution only as the sample grows.
@fig-4b shows the complementary reading for the mixture — once it is properly
penalised from $N = 7$, it earns appreciable weight only where the source data are
genuinely bimodal (the `lnorm_lnorm` panel) and the sample is large enough to reveal
that bimodality, and stays near zero for the unimodal sources. The mixture is a
legitimate competitor under exactly those conditions and a spurious winner at $N = 5$
under none of them.

The downstream effect of the $N = 5$ degeneracy on the estimates themselves is
shown in @fig-3:

```{r}
#| label: fig-3
#| fig-cap: "Median relative bias versus N, faceted by source distribution, coloured by HC proportion. The line and points are the median; the shaded band is the Q25–Q75 spread across datasets and simulations, from the committed bias_by_source_summary.csv. Bias is on a symmetric-log axis (signed, with a heavy positive tail at small N). The large positive bias at N = 5 across the unimodal panels is the mixture degeneracy propagating into the model-averaged HC~x~ estimate; it is largest by far for gamma-sourced data, whose extreme left tail is where the spuriously-weighted mixture over-predicts the low HC~x~ quantiles most severely."
#| fig-width: 8
#| fig-height: 5.5
