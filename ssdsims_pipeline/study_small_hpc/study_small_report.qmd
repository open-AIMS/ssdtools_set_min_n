...
subtitle: "A worked `ssdsims` case study re-examining the 2024 addendum's minimum-sample-size question"
date: today
format:
  html:
    toc: true
...
# and the per-source summaries used in "What the pipeline surfaced":
...
This report demonstrates the [`ssdsims`](https://poissonconsulting.github.io/ssdsims/)
simulation pipeline [@ssdsims] by using it to re-examine the minimum-sample-size
question first investigated in the 2024 addendum to the joint SSD-modelling
investigation [@fisher_addendum_2024]. The addendum is the worked example; the
subject of the report is the pipeline and what running it surfaces about the
behaviour of species sensitivity distribution (SSD) HC~x~ estimation at small
sample sizes.

It is **not** a revision of the 2024 addendum, and it does not supersede that
document's recommendations. It reproduces the addendum's *design* on a smaller,
fully reproducible footing (41 `ssddata` datasets rather than the addendum's
353-dataset combined corpus), so that the estimation behaviour — and one criterion
artefact behind it — can be inspected end to end from committed inputs.

> **Author role.** The author co-authored both the current ANZG derivation method
> [@warne_method_2025] and the `ssdtools` package [@thorley_ssdtools_2025], and
> authored the 2024 addendum [@fisher_addendum_2024] and its simulation code. That
> original simulation code is no longer available, a fact that is itself central to
> the argument in [What the pipeline surfaced: the AICc degeneracy].
...
Recommendation #5 has consequences for the sample-size minima then in force. The
current ANZG minimum is 5 species [@warne_revised_2018; @anzg_2020]. The default
...
ANZG method sets the minimum at 6 species (preferably more)
[@warne_method_2025]. The 2024 addendum [@fisher_addendum_2024] reported
simulation studies designed to inform a single default minimum across both the
two-parameter unimodal distributions and the five-parameter mixture, and
recommended a single minimum of $N = 6$. This report re-runs that style of study
through the `ssdsims` pipeline.
...
here (see [What the pipeline surfaced: the AICc degeneracy]).
...
distribution. This matches the addendum's design: each simulated dataset has a
single true source, not a mixture of sources.
...
(HC1, HC5, HC10, HC20), using the addendum's notation:

- **Relative bias** — $(\widehat{\text{HC}}_x - \text{HC}_x)/\text{HC}_x$, where
  $\widehat{\text{HC}}_x$ is the estimate of the true value $\text{HC}_x$
- **Scaled CI width** — $(\text{UL} - \text{LL})/\text{HC}_x$, where $\text{UL}$
  and $\text{LL}$ are the upper and lower 95% confidence limits
...
as the long whiskers in the top row). For **HC1** the median bias is
**+2.05 at $N = 5$**, flips **negative (−0.24 to −0.32) at $N = 6$–10**, then
returns toward zero. This non-monotonicity is a **direct consequence of the AICc
mixture degeneracy** documented in [What the pipeline surfaced: the AICc
degeneracy]: at $N = 5$, 95.5% of the model-averaged HC estimates (78,272 of
82,000) collapse onto the `lnorm_lnorm` mixture that AICc erroneously rewards when
$n = k$, producing the large positive bias; at $N = 6$ the mixture's AICc weight is
forced to zero, every estimate reverts to a unimodal average, and the bias flips
sign. The apparent "non-monotonic recovery" is therefore an artefact of the
criterion switching regimes between $N = 5$ and $N = 6$, not a property of the
estimator itself.
...
consistent with $N = 5$ being the boundary this study set out to probe. **But
much of this sharp N = 5 / N = 6 split is driven by the AICc mixture degeneracy,
not by sample size itself** — with the degenerate mixture removed the gap shrinks
to a gradual few percentage points (see [Sensitivity to the candidate distribution
set]).
...
## What the pipeline surfaced: the AICc degeneracy

Running the study through the pipeline made a criterion artefact visible that
drives most of the small-N behaviour above. This section sets out the mechanism,
reproduces it, presents the per-source figures that are the direct analogue of the
addendum's Figures 3 and 4, and reports what could and could not be reconciled with
the original study.

### Which datasets represent which source distribution

Each dataset's simulations were generated from its own top-AICc-weighted BCANZ
distribution — its "source" distribution. How cleanly a dataset represents a single
distribution is itself informative, and it determines how much weight the
per-source panels below can bear. The per-dataset weights are extracted from the
cached fits into `dataset_aicc_weights.csv`.
...
unambiguously one distribution. This is why the addendum drew on the much larger
353-dataset corpus — to find clean, high-weight representatives of each source
distribution. The counts of datasets per source are thin here and vary widely
(gamma 4, lgumbel 15, llogis 5, lnorm 10, lnorm_lnorm 1, weibull 6); the
`lnorm_lnorm`-source panel below is a **single dataset** (`anzg_iron_marine`) and
must not be over-read. The full 41 × 6 weight matrix is in
...
essentially all of the AICc weight.

The degeneracy is a property of the **criterion**, not of any `ssdtools` release
or fit setting. `aicc_degeneracy_reprex.R` re-runs the reprex above under
`ssdtools` 1.0.6, 2.0.0 and 2.6.0, and under both each version's native defaults
and the lenient settings (`computable = FALSE`, `at_boundary_ok = TRUE`,
`min_pmix = 0`) that a workflow lowering the minimum might use, extracting the
mixture's full-precision AICc weight at $N = 5, 6, 7$. (The weight is reconstructed
from the `aicc` column as $\exp(-\Delta/2)/\sum_j \exp(-\Delta_j/2)$; the
`ssd_gof()` weight column is rounded to three decimals and would show the tiny
$N = 7$ weight as 0.) $N = 6$ and $N = 7$ use the natural geometric extensions of
the five-point data.

```{r aicc-version-table}
...
`r vc_n7_aicc`). Relaxing `computable`, `at_boundary_ok` and `min_pmix` does not
drop the degenerate $N = 5$ fit: the behaviour cannot be worked around through fit
arguments, because it is the criterion, evaluated outside its valid range, that
rewards the fit.

### Per-source weights: the direct analogue of the addendum's Figure 4

The addendum's Figure 4 tracked, for data generated from each source distribution,
how AICc weights changed with $N$: panel 4A the weight of the source distribution
itself, panel 4B the weight of the `lnorm_lnorm` mixture. The two figures below are
the like-for-like analogue, built from the committed `source_weights_summary.csv`
(mean re-fitted AICc weight per source × $N$ × distribution, averaged over the 500
simulations and over the datasets sharing a source). Panel headers carry the number
of contributing datasets so thin panels are visible.
...
# Numbers referenced in the prose below (all from the committed CSV).
mix5 <- sw |>
...
```{r fig4a}
#| fig-cap: "Analogue of the addendum's Figure 4A: mean re-fitted AICc weight of the SOURCE distribution versus N, one panel per source distribution. At N = 5 the source distribution receives almost no weight in the five unimodal panels — the mixture has taken it (see 4B). The dashed line marks N ≥ 7, where the mixture's AICc becomes valid."
#| fig-width: 8
#| fig-height: 5
fig4a <- sw |>
  filter(distribution == source_dist) |>
  mutate(source_dist = src_factor(source_dist))

ggplot(fig4a, aes(nrow, mean_weight)) +
  geom_vline(xintercept = 7, linetype = 3, colour = "grey60") +
  geom_line(colour = "grey40") +
  geom_point(size = 1.4) +
  facet_wrap(~source_dist, labeller = as_labeller(src_labeller)) +
  scale_x_continuous(breaks = sort(unique(sw$nrow))) +
  scale_y_continuous(limits = c(0, 1)) +
  labs(x = "N (sample size)",
    y = "Mean re-fitted AICc weight\nof the source distribution") +
  theme_bw(base_size = 10)
```

```{r fig4b}
#| fig-cap: "Analogue of the addendum's Figure 4B: mean re-fitted AICc weight of the lnorm_lnorm MIXTURE versus N, faceted by source distribution. The mixture sits at ≈ 1 at N = 5 in every panel — including all five unimodal sources, which did not generate mixture data. It drops to 0 at N = 6 (AICc = +∞) and only becomes meaningful from N = 7 (dashed line). The addendum's Figure 4B showed the opposite: the mixture near zero for unimodal sources at all N."
#| fig-width: 8
#| fig-height: 5
fig4b <- sw |>
  filter(distribution == "lnorm_lnorm") |>
  mutate(source_dist = src_factor(source_dist))

ggplot(fig4b, aes(nrow, mean_weight)) +
  geom_vline(xintercept = 7, linetype = 3, colour = "grey60") +
  geom_line(colour = "grey40") +
  geom_point(size = 1.4) +
  facet_wrap(~source_dist, labeller = as_labeller(src_labeller)) +
...
Figure 4B is the decisive one. At $N = 5$ the mixture is handed a mean re-fitted
weight of `r round(mix5, 2)` across all 41 datasets (`r round(mix5_uni, 2)`
averaged over just the five unimodal sources) — regardless of what generated the
data. The degeneracy is therefore **independent of the source distribution**: it is
a property of the criterion at $n = k$, not evidence of bimodality. The five
unimodal panels carry this argument; the `lnorm_lnorm`-source panel is a single
dataset (`anzg_iron_marine`) and is shown only for completeness. **This is the
opposite of the addendum's Figure 4B**, which showed the mixture receiving low
weight for unimodal sources even at large $N$.

The relative-bias analogue of the addendum's Figure 3 shows the downstream effect:

```{r fig3}
#| fig-cap: "Analogue of the addendum's Figure 3: median relative bias versus N, faceted by source distribution, coloured by HC proportion. Bias is on a symmetric-log axis (signed, with a heavy positive tail at small N). The large positive bias at N = 5 across unimodal panels is the mixture degeneracy propagating into the model-averaged HC~x~ estimate."
#| fig-width: 8
#| fig-height: 5.5
...
ggplot(fig3, aes(nrow, bias_median, colour = HC)) +
...
### The central finding: an irrecoverable result
...
model-averaged HC estimates collapse onto it (Figure 4B; [Results: bias, CI width,
and coverage]).

This is a change in the fitting *engine*, not the criterion. The AICc formula and
its degeneracy are byte-identical across `ssdtools` 1.0.6, 2.0.0 and 2.6.0 (the
table above) [@ssdtools_aicc_issue]; what moved between the 1.0.x and 2.x series is
how often the over-parameterised mixture is actually fitted at $N = 5$. A Monte
Carlo check isolates that — 1,000 samples of $N = 5$ drawn from a unimodal
lognormal source, fitted with `ssd_dists_bcanz()` under each engine (from
`aicc_fit_rate_comparison.csv`, built by `aicc_degeneracy_reprex.R`):

```{r aicc-fit-rate-table}
...
rate_old_lo <- fr_get("1.0.6", "native", "present_ci_lwr")
rate_old_hi <- fr_get("1.0.6", "native", "present_ci_upr")
wt_old <- fr_get("1.0.6", "native", "mean_mixture_weight")
rate_new <- fr_get("2.6.0", "native", "pct_mixture_present")
rate_new_lo <- fr_get("2.6.0", "native", "present_ci_lwr")
rate_new_hi <- fr_get("2.6.0", "native", "present_ci_upr")
wt_new <- fr_get("2.6.0", "native", "mean_mixture_weight")
```

Under the 2024-era engine (`ssdtools` 1.0.6) the mixture is fitted at $N = 5$ in
about `r round(rate_old)`% of these samples (95% CI `r round(rate_old_lo)`–`r round(rate_old_hi)`%),
with mean weight `r round(wt_old, 2)`; under the current engine (2.6.0, the
pipeline's own settings) it is fitted in nearly every case (`r round(rate_new)`%,
95% CI `r round(rate_new_lo)`–`r round(rate_new_hi)`%, mean weight
`r round(wt_new, 2)`). The mean weight tracks the fit rate directly, because a
fitted $N = 5$ mixture almost always wins the $-60$ AICc reward. The pre-existing degeneracy therefore went from
*intermittent* to *near-universal* because the fitting engine improved, not because
the criterion changed.

The addendum's own weights figure (its Figure 4) does not display this behaviour at
all — it shows the mixture near zero for unimodal sources. Because the original
simulation code is unavailable and cannot be recovered from any co-author, we
**cannot determine** whether that reflects the fitting engine of the day, the
sample sizes at which the mixture was evaluated, or both. The discrepancy is
unresolved and is reported here as unresolved.

That inability to reconstruct a published result is itself the argument for the
reproducible pipeline presented here. This is deliberately **not** the claim that
the pipeline "caught a bug the original study missed" — that is not supportable —
nor a claim of a defect in the 2024 work. The point is the irrecoverability: a
result that cannot be regenerated cannot be diagnosed.

A second, smaller instance of the same point surfaced in this study's own metadata.
`ccme_boron`'s source distribution was initially recorded as `gamma` by taking the
argmax of AICc weights **rounded to three decimals**, where gamma and weibull both
display as 0.357. At full precision weibull (0.357472) narrowly beats gamma
(0.356574) by under $10^{-3}$, and that is the distribution the pipeline actually
sampled from — so the committed metadata silently disagreed with the run. This was
a rounding-then-argmax bug, not a genuine tie, and was corrected once identified.
Like the larger discrepancy, it was only catchable because the run and its metadata
were both inspectable.

### Practical messages

Two practical messages follow. First, an AICc-weighted candidate set should not
...
one concrete mechanism behind the poor $N = 5$ behaviour in the headline results:
...
The degeneracy above raises the obvious question: how much of the $N = 5$ failure
is the *estimator* running out of data, and how much is the *criterion* rewarding
a five-parameter mixture it should have excluded at that $N$? This cannot be
...
~30 min (`../study_small_nomix/README.md`); its committed summary feeds the table and
figure below.
...
**The cliff is largely an artefact of the mixture.** With the degenerate
component gone, N = 5 coverage is `r n5_min`–`r n5_max` across the four
proportions — a world away from the 0.58–0.78 seen with the full BCANZ set — and
moving from N = 5 to N = 6 buys only about **`r mean_gap_pts` percentage points**
of coverage on average, with a **smooth, monotonic** rise through N = 8 rather
than a step. The sharp N = 5-versus-N = 6 discontinuity in the headline results
was therefore substantially the AICc criterion switching regimes (mixture weight
$\approx 1$ at N = 5, forced to 0 at N = 6), not an intrinsic property of HC~x~
estimation at N = 5.

Two caveats keep this honest. First, **every cell still under-covers** (~0.85–0.90
against the nominal 0.95, even at N = 8) — a separate small-sample limitation of
the `weighted_samples` bootstrap that removing the mixture does *not* fix.
Second, N = 5 remains the **least precise** case: its median relative CI width at
HC1 is `r w5` versus `r w6` at N = 6, so smaller samples still pay in interval
width and (mostly) bias even when coverage is close. The practical conclusion is
narrower than the headline "N = 5 is a distinct failure regime": once a sane
candidate set is used (no $k \ge N - 1$ distribution), N = 5 is *worse* than N = 6
only gradually, not catastrophically.
...
The value of an additional species falls away quickly once the criterion artefact
is set aside. The table below reports, for the full BCANZ run, the mean coverage and
mean relative CI width at each $N$, and the marginal change per additional species
across the (uneven) grid.

```{r sec8-marginal}
#| tbl-cap: "Marginal value of an additional species in the full BCANZ run. Coverage and relative CI width are averaged over the four HC proportions at each N; the per-species columns divide the change from the previous grid point by the number of species added. The 5→6 coverage gain is inflated by the AICc mixture degeneracy (see text)."
marg_tbl |>
  transmute(
    N = nrow,
    `Mean coverage` = signif(coverage, 3),
    `Coverage gain / species` = dplyr::case_when(
      is.na(cov_gain_per_sp) ~ "—",
      abs(100 * cov_gain_per_sp) < 0.1 ~ sprintf("%+.2f pts", 100 * cov_gain_per_sp),
      TRUE ~ sprintf("%+.1f pts", 100 * cov_gain_per_sp)),
...
Three things follow, and none of them supports raising the minimum.
...
$N = 6$ is `r sprintf("%+.1f", gain_pts(6))` percentage points per species — by far
the largest step in the table — but this is the mixture weight collapsing from
...
buys only about `r mean_gap_pts` percentage points per species, comparable to the
`r sprintf("%+.1f", gain_pts(7))`-point $N = 6 \to N = 7$ step, and the rise through
$N = 8$ is smooth rather than a cliff. The specific evidence that appeared to
support a step change at the minimum does not say what it was taken to say.

Second, **returns diminish sharply and there is no distinct jump anywhere on the
grid.** Beyond $N = 8$ the per-species coverage gain is under one point
...
Taken together, the data support **no change to the minimum sample size.** The
$N = 6$ minimum of @warne_method_2025 remains supportable — nothing here argues for
lowering it, and $N = 5$ remains the least precise case in interval width — but the
sharp $N = 5 / N = 6$ evidence that seemed to argue *for* a minimum at $6$ turns out
to be largely a criterion artefact, and the smooth, always-under-covering behaviour
above it gives no threshold at which to set a higher one. Because each additional
species is an additional toxicity test, a recommendation to raise the minimum would
need positive evidence of a threshold; these results do not provide one.
...
The existing global `nrow >= 6` guard in `ssd_fit_dists()` is a blunt instrument and
is not a substitute. It does not prevent the degeneracy at the $N = 6$ it permits —
...
bite; the global row check only hides one instance of it.
