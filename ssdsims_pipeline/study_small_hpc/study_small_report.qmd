...
> the argument in [The AICc mixture degeneracy].
...
here (see [The AICc mixture degeneracy]).
...
$N = 6$ and reaches about 0.93–0.94 by $N = 26$. $N = 5$ is the most deficient
case, and most of the shortfall is recovered by $N = 6$.

Confidence intervals are wide at small $N$. The median relative CI width is about
41× the true HC1 value at $N = 5$, falling to roughly 3.9× at $N = 26$. Even at
$N = 26$ the low-proportion intervals remain wide, reflecting that a small sample
cannot precisely estimate an extreme lower-tail quantile.

Bias is dominated by a heavy positive tail. Median bias is modest, but the upper
tail reaches order 10^8^ at the smallest $N$ and lowest proportions (visible as the
long whiskers in the top row of @fig-main). For HC1 the median bias is +2.05 at
$N = 5$, turns
negative (−0.24 to −0.32) at $N = 6$–10, then returns toward zero. This
non-monotonicity is a direct consequence of the AICc mixture degeneracy documented
in [The AICc mixture degeneracy]: at $N = 5$, 95.5% of the model-averaged HC
estimates (78,272 of 82,000) collapse onto the `lnorm_lnorm` mixture that AICc
erroneously rewards when $n = k$, producing the large positive bias; at $N = 6$ the
mixture's AICc weight is forced to zero, every estimate reverts to a unimodal
average, and the bias changes sign. The non-monotonic recovery is therefore an
artefact of the criterion switching regimes between $N = 5$ and $N = 6$, rather
than a property of the estimator itself.

Across all three metrics, $N = 5$ is a distinct failure regime and $N = 6$ is the
point at which behaviour becomes substantially more reliable, consistent with
$N = 5$ being the boundary this study set out to probe. Much of this sharp
$N = 5$ / $N = 6$ split is, however, driven by the AICc mixture degeneracy rather
...
These AICc values are fixed by the criterion alone. Each two-parameter candidate
receives a correction of $2\cdot2\cdot3/(5-2-1) = +6$ and the mixture a correction
of $2\cdot5\cdot6/(5-5-1) = -60$: a 66-point swing between the mixture and its
competitors, computed entirely from the integers $n$ and $k$ and set against AIC
differences that, on five observations, are of order $0.1$. Neither the $-60$ at
$n = 5$ nor the $+\infty$ at $n = 6$ depends on the log-likelihood, the optimiser,
or the data. This is why `aicc_version_comparison.csv` returns an identical mixture
weight in every row (@tbl-aicc-version), and why relaxing `computable`,
`at_boundary_ok` or `min_pmix` changes nothing: the ranking is settled before the
data are consulted.

To confirm that this holds beyond the single reprex dataset,
`aicc_degeneracy_reprex.R` re-runs the reprex above under
...
```{r}
#| label: fig-4a
#| fig-cap: "Analogue of the addendum's Figure 4A: re-fitted AICc weight of the SOURCE distribution versus N, one panel per source distribution. Each point is one dataset's mean weight over its 500 simulations (between-dataset spread); each grey line is that dataset's 10th–90th percentile across those simulations (between-simulation spread). At N = 5 the source distribution receives almost no weight in the five unimodal panels — the mixture has taken it (see @fig-4b). The dashed line marks N ≥ 7, where the mixture's AICc becomes valid."
#| fig-width: 8
#| fig-height: 5
...
```{r}
#| label: fig-4b
#| fig-cap: "Analogue of the addendum's Figure 4B: re-fitted AICc weight of the lnorm_lnorm MIXTURE versus N, faceted by source distribution. Each point is one dataset's mean weight over its 500 simulations; each grey line is that dataset's 10th–90th simulation percentile. At N = 5 every point sits at ≈ 1 with a negligible range in every panel — including all five unimodal sources, which did not generate mixture data. The weight drops to exactly 0 at N = 6 (AICc = +∞) and only becomes meaningful from N = 7 (dashed line). The addendum's Figure 4B showed the opposite: the mixture near zero for unimodal sources at all N."
#| fig-width: 8
#| fig-height: 5
weight_plot("lnorm_lnorm", "Re-fitted AICc weight\nof the lnorm_lnorm mixture")
```

@fig-4b is the most direct evidence. At $N = 5$ the mixture receives a mean
re-fitted weight of `r round(mix5, 2)` across all 41 datasets (`r round(mix5_uni, 2)`
averaged over just the five unimodal sources), regardless of what generated the
data. The spread behind that mean is what makes the case: the median re-fitted
mixture weight at $N = 5$ is exactly $1$ for `r if (mix5_all_med1) "every one of the
41 datasets" else "almost every dataset"`, and for `r 41 - mix5_n_spread` of them the
entire 10th-to-90th-percentile band across the 500 simulations sits at $\ge 0.99$ —
both the between-dataset and the between-simulation spread are negligible. The single
exception is `anzg_iron_marine` (the lone `lnorm_lnorm` source), whose mixture
genuinely competes; its panel is the only one showing a visible range at $N = 5$. The
mean is pulled below $1$ (to `r round(mix5, 2)`) only by a thin tail of simulations in
which the mixture fit failed numerically and took weight $0$ — not by any real spread
in the weight the criterion assigns. A weight of $\approx 1$ with this little
variation, across every dataset and almost every simulation and irrespective of what
generated the data, is the strongest available demonstration that the degeneracy is
mechanical, a property of the criterion at $n = k$, rather than data-dependent
evidence of bimodality. It is the like-for-like complement to the criterion-not-fit
argument in [Findings and recommendations]. The five unimodal panels carry this
argument; the `lnorm_lnorm`-source panel is a single dataset and is shown only for
completeness. This is the opposite of the addendum's Figure 4B, which showed the
mixture receiving low weight for unimodal sources even at large $N$.

The relative-bias analogue of the addendum's Figure 3 (@fig-3) shows the downstream
effect:
...
its degeneracy are byte-identical across `ssdtools` 1.0.6, 2.0.0 and 2.6.0
(@tbl-aicc-version) [@ssdtools_aicc_issue]; what moved between the 1.0.x and 2.x
series is
...
Under the 2024-era engine (`ssdtools` 1.0.6; @tbl-fit-rate) the mixture is fitted at
$N = 5$ in about `r round(rate_old)`% of these samples (95% CI `r round(rate_old_lo)`–`r round(rate_old_hi)`%),
...
fitted $N = 5$ mixture almost always wins the $-60$ AICc reward. The pre-existing
degeneracy therefore went from intermittent to near-universal because the fitting
engine improved, not because the criterion changed.

The addendum's own weights figure (its Figure 4) does not display this behaviour at
all: it shows the mixture near zero for unimodal sources. The criterion cannot
produce a near-zero mixture weight at $N = 5$ or $N = 6$ while the mixture is in the
candidate set, because the arithmetic above forces its weight to $\approx 1$ and
then to exactly $0$ irrespective of the data. The addendum's Figure 4 therefore
displays behaviour the criterion could not have produced at those sample sizes
unless the mixture was absent from the candidate set when the figure was made.
Because the original simulation code is unavailable and cannot be recovered from any
co-author, whether the mixture was so absent cannot be determined; the discrepancy —
whether it reflects the fitting engine of the day, the sample sizes at which the
mixture was evaluated, or both — is reported here as unresolved.

That inability to reconstruct a published result is itself the argument for the
reproducible pipeline presented here. This is not a claim that the pipeline "caught
a bug the original study missed" — that is not supportable — nor a claim of a defect
in the 2024 work. The point is the irrecoverability: a result that cannot be
regenerated cannot be diagnosed.
