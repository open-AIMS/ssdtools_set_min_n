...
`ssddata` datasets; it does not replicate the 353-dataset EnviroTox datasets. One reason for this was a desire to use real, curated ssd datasets in the testing, now that the `ssddata` package was updated. The 
consequence is that some source distributions are represented by very few datasets
here (see **Source representation** below).

**Single-source generating design.** For each dataset, the top-AICc-weighted BCANZ distribution (`r ssdtools::ssd_dists_bcanz()`)
from the real-data fit was treated as that dataset's single
...
unambiguously one distribution. The counts of datasets per source vary
(gamma 4, lgumbel 15, llogis 5, lnorm 10, lnorm_lnorm 1, weibull 6), so the
...
from the recommended weighted-sample bootstrap [@fox_final_2024]. All fits were
produced under `ssdtools` 2.6.0 (the most recent CRAN release, 2026-03-05), in
which the AICc small-sample degeneracy described in [The AICc mixture degeneracy]
is present, as it is in all earlier versions.
...
## Results
### Bias, CI width, and coverage overall all datasets
...
### AICc weights and bias for each source distribution

The aggregate metrics in the previous section average over all 41
...
For data generated from each source distribution, @fig-source-weights shows how the
re-fitted AICc weights change with $N$.
...
```{r}
#| label: fig-source-weights
#| fig-cap: "Re-fitted AICc weight versus N, faceted by source distribution. (A) weight of the source distribution itself; (B) weight of the lnorm_lnorm mixture. In both panels each point is one dataset's mean weight over its 500 simulations (between-dataset spread, from the committed fit_weights_summary.csv), and each grey line is that dataset's 10th–90th percentile across those simulations (between-simulation spread, from fit_weights_quantiles.csv); the dashed line marks N = 7, where the five-parameter mixture's AICc first becomes valid. Each panel header carries the number of contributing datasets (from source_weights_summary.csv) so that thin panels — the lnorm_lnorm source has only one — are visible."
#| fig-width: 8
#| fig-height: 9
# Stacked with patchwork (the repo's convention, as in make_report_outputs.R):
# two facet grids share an identical x scale, so the top panel drops its x-axis
# title and only the bottom carries "N (sample size)". Panels tagged (A)/(B).
pa <- weight_plot("source", "Re-fitted AICc weight\nof the source distribution") +
  labs(x = NULL)
pb <- weight_plot("lnorm_lnorm", "Re-fitted AICc weight\nof the lnorm_lnorm mixture")
(pa / pb) +
  plot_annotation(tag_levels = "A", tag_prefix = "(", tag_suffix = ")")
```

At $N \ge 7$ — the dashed line, where the mixture's AICc becomes valid — panel A of
@fig-source-weights reads the way model averaging should. The weight on the source
distribution rises with $N$: at small $N$ a handful of observations cannot distinguish
one candidate distribution from another, so the weight is spread diffusely across the
candidates, and it concentrates on the generating distribution only as the sample
grows. Recovery is incomplete even at $N = 26$, because model averaging over six
candidates spreads weight and the real-data weights are themselves diffuse
([Source representation]). Panel B shows the complementary reading for the mixture:
from $N \ge 7$, where it is properly penalised, a non-trivial mixture weight requires
both genuine bimodality in the source data and a sample large enough to detect it.
Only `anzg_iron_marine` — the sole bimodal source among the 41 ([Source
representation]) — attracts appreciable mixture weight, and only at larger $N$; the
five unimodal sources stay near zero. This is the expected and correct behaviour of
the criterion: the mixture is a legitimate competitor under exactly those conditions.
...
$N = 5$ the `lnorm_lnorm` mixture takes essentially all the weight in panel B of
@fig-source-weights, in every source panel — including the five unimodal sources, which
generated no mixture data — so the weight on the source distribution in panel A
collapses to near zero and the bias in @fig-3 jumps to its largest positive values. At $N = 6$ the mixture takes exactly none
...
(@fig-source-weights, @fig-3) to show that the degeneracy is a property of the criterion
...
This is reproducible in a few lines under `ssdtools` 2.6.0 and earlier, and is
recorded as a `ssdtools` software issue [@ssdtools_aicc_issue]:

```r
library(ssdtools)                                     # 2.6.0 and earlier
...
regardless of what generated the data (panel B of @fig-source-weights). The spread behind that mean is what
...
five unimodal panels in panel B of @fig-source-weights carry this argument; the `lnorm_lnorm`-source panel is
...
under the `ssdtools` 2.x fitting engine. At $N = 5$ the engine fits the
...
model-averaged HC estimates collapse onto it (panel B of @fig-source-weights; [Results: bias, CI width,
...
samples, with mean weight `r round(wt_old, 2)`; under the 2.6.0 engine (the
