...
The two-panel weight figure below tracks, for data generated from each source distribution,
how the re-fitted AICc weights change with $N$: panel A the weight assigned to the
source distribution itself, panel B the weight assigned to the `lnorm_lnorm` mixture.
Both panels are built from the committed `source_weights_summary.csv` (mean re-fitted AICc
...
#| fig-cap: "Re-fitted AICc weight versus N, faceted by source distribution. (A) weight of the source distribution itself; (B) weight of the lnorm_lnorm mixture. In both panels each point is one dataset's mean weight over its 500 simulations (between-dataset spread), and each grey line is that dataset's 10th–90th percentile across those simulations (between-simulation spread); the dashed line marks N = 7, where the five-parameter mixture's AICc first becomes valid. From N ≥ 7 the source-distribution weight (A) rises with N as more observations let the criterion discriminate the generating distribution from its competitors, while the mixture (B) earns appreciable weight only where the source data are genuinely bimodal and the sample is large enough to detect it. The near-zero source weight and ≈ 1 mixture weight at N = 5 in every panel — including the five unimodal sources, which generated no mixture data — and the shift at N = 6 are explained in [The AICc mixture degeneracy]."
#| fig-width: 8
#| fig-height: 9
